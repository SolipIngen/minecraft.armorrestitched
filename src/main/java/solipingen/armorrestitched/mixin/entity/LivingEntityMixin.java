...
public abstract class LivingEntityMixin extends Entity {
    ...
    private void injectedBaseTick(CallbackInfo cbi) {
        for (EquipmentSlot slot : EquipmentSlot.values()) {
            ...
            if (trim.getMaterial().matchesKey(ArmorTrimMaterials.QUARTZ)) {
            ...
            else if (trim.getMaterial().matchesKey(ArmorTrimMaterials.REDSTONE)) {
    ...
    @ModifyVariable(method = "tickFallFlying", at = @At("STORE"), index = 2)
    private ItemStack modifiedElytraStack(ItemStack itemStack) {
        if (itemStack.getItem() instanceof ElytraItem && !ElytraItem.isUsable(itemStack) && itemStack.get(DataComponentTypes.TRIM) != null) {
            itemStack.remove(DataComponentTypes.TRIM);
            if (!this.isSilent()) {
    ...
    private void injectedDamage(DamageSource source, float amount, CallbackInfoReturnable<Boolean> cbireturn) {
        ...
        if (!blockingBl) {
            ...
            for (EquipmentSlot slot : EquipmentSlot.values()) {
                ...
                ItemStack equippedStack = ((LivingEntity)(Object)this).getEquippedStack(slot);
                ArmorTrim trim = equippedStack.get(DataComponentTypes.TRIM);
                if (trim != null && trim.getMaterial().matchesKey(ArmorTrimMaterials.AMETHYST)) {
                    i++;
                }
            }
            if (i > 0) {
                ...
                for (Entity entity : amethystSoundEntities) {
                    ...
                    if (entity instanceof MobEntity && entity.getWorld() instanceof ServerWorld) {
                        ...
                        else if (mobEntity.getTarget() != null && !mobEntity.getType().isIn(EntityTypeTags.UNDEAD) && (mobEntity instanceof HostileEntity || mobEntity.getTarget() == ((LivingEntity)(Object)this))) {
    ...
    private void injectedProtectionAmount(DamageSource source, float amount, CallbackInfoReturnable<Float> cbireturn) {
        int i;
        int j;
        float f;
        float g;
        ...
        if (source.isIn(DamageTypeTags.BYPASSES_EFFECTS)) {
            cbireturn.setReturnValue(amount);
        }
        if (((LivingEntity)(Object)this).hasStatusEffect(StatusEffects.RESISTANCE) && !source.isIn(DamageTypeTags.BYPASSES_RESISTANCE) && (h = (g = amount) - (amount = Math.max((f = amount * (float)(j = 25 - (i = (((LivingEntity)(Object)this).getStatusEffect(StatusEffects.RESISTANCE).getAmplifier() + 1) * 5))) / 25.0f, 0.0f))) > 0.0f && h < 3.4028235E37f) {
            if (((LivingEntity)(Object)this) instanceof ServerPlayerEntity serverPlayerEntity) {
                serverPlayerEntity.increaseStat(Stats.DAMAGE_RESISTED, Math.round(h * 10.0f));
        ...
        i = EnchantmentHelper.getProtectionAmount(((LivingEntity)(Object)this).getAllArmorItems(), source);
        if (source.getSource() instanceof PersistentProjectileEntity persistentProjectileEntity) {
            i = Math.round((1.0f - 0.07f*persistentProjectileEntity.getPierceLevel())*i);
        }
        else if (!(source.getSource() instanceof PersistentProjectileEntity) && source.getAttacker() instanceof LivingEntity) {
            int breachLevel = EnchantmentHelper.getEquipmentLevel(Enchantments.BREACH, (LivingEntity)source.getAttacker());
            i = Math.round((1.0f - 0.07f*breachLevel)*i);
        }
        if (i > 0) {
    ...
    private void injectedDamageEquipment(DamageSource source, float amount, EquipmentSlot[] slots, CallbackInfo cbi) {
        ...
        for (EquipmentSlot equipmentSlot : slots) {
            ...
            if (!(itemStack.getItem() instanceof ArmorItem) || !itemStack.isIn(ItemTags.TRIMMABLE_ARMOR) || !itemStack.takesDamageFrom(source)) continue;
    ...
    private void injectedAddStatusEffectInstance(StatusEffectInstance effect, @Nullable Entity source, CallbackInfoReturnable<Boolean> cbireturn) {
        ...
        for (EquipmentSlot slot : EquipmentSlot.values()) {
            ...
            ArmorTrim trim = equippedStack.get(DataComponentTypes.TRIM);
            if (trim == null) continue;
            if (trim.getMaterial().matchesKey(ArmorTrimMaterials.REDSTONE)) {
