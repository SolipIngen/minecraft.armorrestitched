...
public abstract class LivingEntityMixin extends Entity {
    ...
    @Unique private static final Vec3d[] REDSTONE_CHARGE_COLORS = Util.make(new Vec3d[16], colors -> {
        for (int i = 0; i <= 15; ++i) {
            float f = i / 15.0f;
    ...
    private void injectedBaseTick(CallbackInfo cbi) {
        ...
        if (this.getWorld() instanceof ServerWorld && this.redstoneCharged && this.redstoneChargeTime % 4 == 0 && this.redstoneChargePower > 0) {
            ((ServerWorld)this.getWorld()).spawnParticles(new DustParticleEffect(REDSTONE_CHARGE_COLORS[this.redstoneChargePower].toVector3f(), 0.5f), this.getX(), this.getBodyY(this.redstoneParticleYScale), this.getZ(), this.random.nextInt(this.redstoneChargePower), 0.0, 0.0, 0.0, this.random.nextDouble());
        }
    ...
    private void injectedWriteCustomDataToNbt(NbtCompound nbt, CallbackInfo cbi) {
        ...
        nbt.putDouble("RedstoneParticleYScale", this.redstoneParticleYScale);
    }
    ...
    private void injectedReadCustomDataFromNbt(NbtCompound nbt, CallbackInfo cbi) {
        ...
        this.redstoneParticleYScale = nbt.getDouble("RedstoneParticleYScale");
    }
