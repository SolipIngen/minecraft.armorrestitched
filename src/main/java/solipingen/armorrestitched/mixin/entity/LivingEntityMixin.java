...
import solipingen.armorrestitched.mixin.accessors.entity.EntityAttributeModifierAccessor;
...
public abstract class LivingEntityMixin extends Entity {
    ...
    private void injectedBaseTick(CallbackInfo cbi) {
        for (EquipmentSlot slot : EquipmentSlot.values()) {
            ...
            Optional<ArmorTrim> trimOptional = ArmorTrim.getTrim(this.getWorld().getRegistryManager(), equippedStack, false);
            ...
            if (this.getWorld() instanceof ServerWorld && this.redstoneCharged && this.redstoneChargeTime % 4 == 0 && this.redstoneChargePower > 0) {
                ((ServerWorld)this.getWorld()).spawnParticles(new DustParticleEffect(REDSTONE_CHARGE_COLORS[this.redstoneChargePower].toVector3f(), 0.5f), this.getX(), this.getBodyY(0.5), this.getZ(), this.random.nextInt(this.redstoneChargePower), 0.0, 0.0, 0.0, this.random.nextDouble());
            }
        }
    ...
    private ItemStack modifiedElytraStack(ItemStack itemStack) {
        if (itemStack.getItem() instanceof ElytraItem && !ElytraItem.isUsable(itemStack) && ArmorTrim.getTrim(this.getWorld().getRegistryManager(), itemStack, false).isPresent()) {
                ...
                for (Entity entity : amethystSoundEntities) {
                    ...
                    if (entity instanceof MobEntity && entity.getWorld() instanceof ServerWorld) {
                        ...
                        if (mobEntity.getTarget() == null && mobEntity.getGroup() != EntityGroup.UNDEAD) {
                            ...
                            duration *= mobEntity instanceof WardenEntity ? 2 : 1;
    ...
    private Multimap<EntityAttribute, EntityAttributeModifier> redirectedAttributeModifiers(ItemStack itemStack, EquipmentSlot slot) {
        ...
        Optional<ArmorTrim> trimOptional = ArmorTrim.getTrim(this.getWorld().getRegistryManager(), itemStack, false);
        if (trimOptional.isPresent()) {
            ...
            if (itemStack.isOf(Items.ELYTRA)) {
                if (trimMaterial.matchesKey(ArmorTrimMaterials.COPPER)) {
                    ...
                    for (EntityAttributeModifier currentAttributeModifier : currentAttributeModifiers) {
                        double addition = ((EntityAttributeModifierAccessor)currentAttributeModifier).getName().matches("Armor toughness") ? 0.25 : 1.0;
                        EntityAttributeModifier boostedAttributeModifier = new EntityAttributeModifier(currentAttributeModifier.getId(), ((EntityAttributeModifierAccessor)currentAttributeModifier).getName(), currentAttributeModifier.getValue() + addition, currentAttributeModifier.getOperation());
                    ...
                    currentAttributeModifiers.addAll(itemStack.getAttributeModifiers(slot).get(EntityAttributes.GENERIC_ARMOR_TOUGHNESS));
                    for (EntityAttributeModifier currentAttributeModifier : currentAttributeModifiers) {
                        double addition = ((EntityAttributeModifierAccessor)currentAttributeModifier).getName().matches("Armor toughness") ? 0.5 : 2.0;
                        EntityAttributeModifier boostedAttributeModifier = new EntityAttributeModifier(currentAttributeModifier.getId(), ((EntityAttributeModifierAccessor)currentAttributeModifier).getName(), currentAttributeModifier.getValue() + addition, currentAttributeModifier.getOperation());
                        multimap.remove(EntityAttributes.GENERIC_ARMOR, currentAttributeModifier);
                        multimap.put(EntityAttributes.GENERIC_ARMOR, boostedAttributeModifier);
                        multimap.remove(EntityAttributes.GENERIC_ARMOR_TOUGHNESS, currentAttributeModifier);
                    ...
                    currentAttributeModifiers.addAll(itemStack.getAttributeModifiers(slot).get(EntityAttributes.GENERIC_ARMOR_TOUGHNESS));
                    for (EntityAttributeModifier currentAttributeModifier : currentAttributeModifiers) {
                        double addition = ((EntityAttributeModifierAccessor)currentAttributeModifier).getName().matches("Armor toughness") ? 1.0 : 3.0;
                        EntityAttributeModifier boostedAttributeModifier = new EntityAttributeModifier(currentAttributeModifier.getId(), ((EntityAttributeModifierAccessor)currentAttributeModifier).getName(), currentAttributeModifier.getValue() + addition, currentAttributeModifier.getOperation());
                        multimap.remove(EntityAttributes.GENERIC_ARMOR, currentAttributeModifier);
                        multimap.put(EntityAttributes.GENERIC_ARMOR, boostedAttributeModifier);
                        multimap.remove(EntityAttributes.GENERIC_ARMOR_TOUGHNESS, currentAttributeModifier);
                    ...
                    Collection<EntityAttributeModifier> currentAttributeModifiers = itemStack.getAttributeModifiers(slot).get(EntityAttributes.GENERIC_ARMOR);
                    for (EntityAttributeModifier currentAttributeModifier : currentAttributeModifiers) {
                        EntityAttributeModifier boostedAttributeModifier = new EntityAttributeModifier(currentAttributeModifier.getId(), ((EntityAttributeModifierAccessor)currentAttributeModifier).getName(), currentAttributeModifier.getValue() + 2.0, currentAttributeModifier.getOperation());
                        multimap.remove(EntityAttributes.GENERIC_ARMOR, currentAttributeModifier);
                        multimap.put(EntityAttributes.GENERIC_ARMOR, boostedAttributeModifier);
                    }
                }
                    ...
                    for (EntityAttributeModifier currentAttributeModifier : currentAttributeModifiers) {
                        double addition = ((EntityAttributeModifierAccessor)currentAttributeModifier).getName().matches("Armor knockback resistance") ? 0.025 : (((EntityAttributeModifierAccessor)currentAttributeModifier).getName().matches("Armor toughness") ? 1.0 : 3.0);
                        EntityAttributeModifier boostedAttributeModifier = new EntityAttributeModifier(currentAttributeModifier.getId(), ((EntityAttributeModifierAccessor)currentAttributeModifier).getName(), currentAttributeModifier.getValue() + addition, currentAttributeModifier.getOperation());
                        multimap.remove(EntityAttributes.GENERIC_ARMOR, currentAttributeModifier);
                        multimap.put(EntityAttributes.GENERIC_ARMOR, boostedAttributeModifier);
                        multimap.remove(EntityAttributes.GENERIC_ARMOR_TOUGHNESS, currentAttributeModifier);
            ...
            if (trimMaterial.matchesKey(ArmorTrimMaterials.DIAMOND)) {
                ...
                for (EntityAttributeModifier currentAttributeModifier : currentAttributeModifiers) {
                    EntityAttributeModifier boostedAttributeModifier = new EntityAttributeModifier(currentAttributeModifier.getId(), ((EntityAttributeModifierAccessor)currentAttributeModifier).getName(), currentAttributeModifier.getValue() + 1.0, currentAttributeModifier.getOperation());
                    multimap.remove(EntityAttributes.GENERIC_ARMOR_TOUGHNESS, currentAttributeModifier);
                    multimap.put(EntityAttributes.GENERIC_ARMOR_TOUGHNESS, boostedAttributeModifier);
                }
            }
                ...
                for (EntityAttributeModifier currentAttributeModifier : currentAttributeModifiers) {
                    EntityAttributeModifier boostedAttributeModifier = new EntityAttributeModifier(currentAttributeModifier.getId(), ((EntityAttributeModifierAccessor)currentAttributeModifier).getName(), currentAttributeModifier.getValue() + 0.05, currentAttributeModifier.getOperation());
                    multimap.remove(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE, currentAttributeModifier);
                    multimap.put(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE, boostedAttributeModifier);
                }
            }
