...
public abstract class SpellcastingIllagerEntityMixin extends IllagerEntity {
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData, @Nullable NbtCompound entityNbt) {
        ...
        if (spawnReason == SpawnReason.STRUCTURE || this.isPatrolLeader()) {
            int level = this.random.nextFloat() < 0.2f*this.world.getDifficulty().getId() + 0.2f*difficulty.getClampedLocalDifficulty() ? 4 : 3;
    ...
    protected void initEquipment(Random random, LocalDifficulty localDifficulty) {
        ...
        for (EquipmentSlot slot : EquipmentSlot.values()) {
            if (slot.getType() != EquipmentSlot.Type.ARMOR) continue;
            if (slot == EquipmentSlot.HEAD && this.isPatrolLeader()) continue;
            Item armorItem = SpellcastingIllagerEntityMixin.getModEquipmentForSlot(slot, random.nextFloat() > 0.2f*this.world.getDifficulty().getId() + 0.2f*localDifficulty.getClampedLocalDifficulty() ? 4 : 3);
            this.equipStack(slot, new ItemStack(armorItem));
