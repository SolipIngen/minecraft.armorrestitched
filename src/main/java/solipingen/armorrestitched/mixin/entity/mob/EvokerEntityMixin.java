...
public abstract class EvokerEntityMixin extends IllagerEntity {
    ...
    protected void initEquipment(Random random, LocalDifficulty localDifficulty) {
        ...
        for (EquipmentSlot slot : EquipmentSlot.values()) {
            if (!slot.isArmorSlot()) continue;
            if (slot == EquipmentSlot.HEAD && this.isPatrolLeader()) continue;
            Item armorItem = EvokerEntityMixin.getModEquipmentForSlot(slot, random.nextFloat() < 0.025f*localDifficulty.getGlobalDifficulty().getId() + 0.02f*localDifficulty.getClampedLocalDifficulty() ? 4 : 3);
            if (armorItem != null) {
                this.equipStack(slot, new ItemStack(armorItem));
            }
        }
