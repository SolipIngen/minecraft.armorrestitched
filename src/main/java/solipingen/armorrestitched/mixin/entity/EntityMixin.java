...
public abstract class EntityMixin implements Nameable, EntityLike, CommandOutput, EntityInterface {
    ...
    private void injectedLimitFallDistance(CallbackInfo cbi) {
        if (((Entity)(Object)this instanceof LivingEntity livingEntity) && EnchantmentHelper.getEquipmentLevel(ModEnchantments.IMPACT_PROTECTION, livingEntity) > 0) {
            this.fallDistance *= (1.0f - 0.005f*EnchantmentHelper.getEquipmentLevel(ModEnchantments.IMPACT_PROTECTION, livingEntity));
        }
    }
    ...
    private void injectedOnStruckByLightning(ServerWorld world, LightningEntity lightning, CallbackInfo cbi) {
        ...
        if (((Entity)(Object)this) instanceof LivingEntity) {
            ...
            for (EquipmentSlot slot : EquipmentSlot.values()) {
                ...
                if (trim.getMaterial().matchesKey(ArmorTrimMaterials.COPPER)) {
                    damageAmount *= 0.75f;
                    i++;
