...
public class SilkMothEntity extends PassiveEntity implements Flutterer {
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(SILK_MOTH_FLAGS, (byte)0);
    }
    ...
    public static DefaultAttributeContainer.Builder createSilkMothAttributes() {
        return MobEntity.createMobAttributes().add(EntityAttributes.GENERIC_MAX_HEALTH, 4.0).add(EntityAttributes.GENERIC_FLYING_SPEED, 0.5).add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.4);
    }
    ...
    public void writeCustomDataToNbt(NbtCompound nbt) {
        ...
        nbt.putByte("SilkMothFlags", this.dataTracker.get(SILK_MOTH_FLAGS));
        nbt.putInt(TICKS_SINCE_LAYING_EGGS_KEY, this.ticksSinceLayingEggs);
    }

    @Override
    public void readCustomDataFromNbt(NbtCompound nbt) {
        super.readCustomDataFromNbt(nbt);
        this.dataTracker.set(SILK_MOTH_FLAGS, nbt.getByte("SilkMothFlags"));
        this.ticksSinceLayingEggs = nbt.getInt(TICKS_SINCE_LAYING_EGGS_KEY);
    }
    ...
    public class MoveToMulberryGoal extends MoveToTargetPosGoal {
        ...
        public boolean hasReached() {
            ...
            for (Direction direction : Direction.values()) {
                ...
                bl = this.isTargetPos(SilkMothEntity.this.world, blockPos);
                if (bl) {
                    SilkMothEntity.this.world.setBlockState(blockPos, ModBlocks.MULBERRY_SILKWORM_LEAVES.getDefaultState());
                    if (SilkMothEntity.this.world instanceof ServerWorld) {
                        ServerWorld serverWorld = (ServerWorld)SilkMothEntity.this.world;
                        for (int j = 0; j < 8; j++) {
    ...
    public class MoveToLightGoal extends MoveToTargetPosGoal {
        ...
        @Override
        public boolean canStart() {
            return SilkMothEntity.this.world.getLightLevel(SilkMothEntity.this.getBlockPos()) > 0 && this.findTargetPos();
        }
