...
public class ModVillagerProfessions {
    ...
    public static final VillagerProfession DRESSER = ModVillagerProfessions.registerVillagerProfession("dresser",
        RegistryKey.of(RegistryKeys.POINT_OF_INTEREST_TYPE, Identifier.of(ArmorRestitched.MOD_ID, "dresser_poi")), null, null, ModSoundEvents.DRESSER_WORKS);
    ...
    public static void putDresserTradeOffers(Map<RegistryKey<VillagerProfession>, Int2ObjectMap<Factory[]>> originalTradeOffers, MerchantEntity villager) {
        ...
        originalTradeOffers.put(RegistryKey.of(RegistryKeys.VILLAGER_PROFESSION, Identifier.of(ArmorRestitched.MOD_ID, "dresser")), ModVillagerProfessions.copyToFastUtilMap(
            ImmutableMap.of(
                1, new Factory[]{new BuyItemFactory(ModItems.COTTON, 24, 16, 2), new BuyItemFactory(ModItems.FLAX_STRAW, 24, 16, 2),
    ...
    public static void replaceArmorerProfessionToLeveledTrade(Map<RegistryKey<VillagerProfession>, Int2ObjectMap<Factory[]>> originalTradeOffers) {
    ...
    public static void replaceRebalancedArmorerProfessionToLeveledTrade(Map<RegistryKey<VillagerProfession>, Int2ObjectMap<Factory[]>> originalTradeOffers) {
        originalTradeOffers.replace(VillagerProfession.ARMORER, ModVillagerProfessions.copyToFastUtilMap(
            ImmutableMap.of(
                1, new Factory[]{new BuyItemFactory(Items.COAL, 15, 16, 2), new BuyItemFactory(Items.CHARCOAL, 15, 16, 2), 
                    new BuyItemFactory(Items.STICK, 64, 16, 2), new BuyItemFactory(Items.TINTED_GLASS, 2, 16, 2), 
    ...
    public static void replaceLeatherworkerProfessionToLeveledTrade(Map<RegistryKey<VillagerProfession>, Int2ObjectMap<Factory[]>> originalTradeOffers, MerchantEntity villager) {
        ArrayList<DyeItem> dyeItemList = new ArrayList<DyeItem>();
        ArrayList<BlockItem> blockItemList = new ArrayList<BlockItem>();
        ArrayList<BlockItem> carpetItemList = new ArrayList<BlockItem>();
        for (Item item : Registries.ITEM) {
        ...
        originalTradeOffers.replace(VillagerProfession.LEATHERWORKER, ModVillagerProfessions.copyToFastUtilMap(
            ImmutableMap.of(
                ...
                4, new Factory[]{new BuyItemFactory(Items.TURTLE_SCUTE, 1, 12, 30), new BuyItemFactory(Items.ARMADILLO_SCUTE, 12, 12, 30), new BuyItemFactory(Items.FLINT, 13, 12, 30),
    ...
    public static void replaceShepherdProfessionToLeveledTrade(Map<RegistryKey<VillagerProfession>, Int2ObjectMap<Factory[]>> originalTradeOffers, MerchantEntity villager) {
        ArrayList<DyeItem> dyeItemList = new ArrayList<DyeItem>();
        ...
        originalTradeOffers.replace(VillagerProfession.SHEPHERD, ModVillagerProfessions.copyToFastUtilMap(
            ImmutableMap.of(
                ...
                3, new Factory[]{new BuyItemFactory(journeymanDye, 4, 12, 20), new SellItemFactory(journeymanFabric, 1, 16, 10), 
                    new SellItemFactory(journeymanCarpet, 1, 32, 10), new SellItemFactory(journeymanBed, 1, 1, 10)}, 
                4, new Factory[]{new BuyItemFactory(Items.WHEAT, 20, 16, 30), new BuyItemFactory(ModItems.FLAX_STRAW, 20, 16, 30), new SellItemFactory(expertBanner, 1, 3, 15),
                    new SellItemFactory(Items.PAINTING, 2, 1, 15), new SellItemFactory(Items.FLOWER_BANNER_PATTERN, 2, 1, 15)}, 
    ...
    private static VillagerProfession registerVillagerProfession(String name, RegistryKey<PointOfInterestType> poiType, @Nullable ImmutableSet<Item> gatherableItems, @Nullable ImmutableSet<Block> secondaryJobSites, @Nullable SoundEvent workSound) {
        ...
        return Registry.register(Registries.VILLAGER_PROFESSION, Identifier.of(ArmorRestitched.MOD_ID, name),
            new VillagerProfession(Text.translatable("entity." + ArmorRestitched.MOD_ID + ".villager." + name), entry -> entry.matchesKey(poiType), entry -> entry.matchesKey(poiType), gatherableItemsSet, secondaryJobSitesSet, workSound));
    }

    private static PointOfInterestType registerPOI(String name, Block block) {
        return PointOfInterestHelper.register(Identifier.of(ArmorRestitched.MOD_ID, name),
            1, 1, ImmutableSet.copyOf(block.getStateManager().getStates()));
    }

    public static void registerModVillagerProfessions() {
        VillagerInteractionRegistries.registerGiftLootTable(RegistryKey.of(RegistryKeys.VILLAGER_PROFESSION, Identifier.of(ArmorRestitched.MOD_ID, "dresser")),
                RegistryKey.of(RegistryKeys.LOOT_TABLE, Identifier.of(ArmorRestitched.MOD_ID, "gameplay/hero_of_the_village/dresser_gift")));
        VillagerInteractionRegistries.registerGiftLootTable(VillagerProfession.LEATHERWORKER,
                RegistryKey.of(RegistryKeys.LOOT_TABLE, Identifier.of(ArmorRestitched.MOD_ID, "gameplay/hero_of_the_village/leatherworker_gift")));
        VillagerInteractionRegistries.registerGiftLootTable(VillagerProfession.SHEPHERD,
                RegistryKey.of(RegistryKeys.LOOT_TABLE, Identifier.of(ArmorRestitched.MOD_ID, "gameplay/hero_of_the_village/shepherd_gift")));
    ...
    public static class SellDyedItemFactory implements Factory {
        ...
        public TradeOffer create(Entity entity, Random random) {
            ...
            if (this.enchanted) {
                ...
                itemStack = EnchantmentHelper.enchant(random, itemStack, i, dynamicRegistryManager, optional);
            }
            int j = this.basePrice;
            if (this.enchanted) {
                j = Math.min(this.basePrice + i, 64);
            }
            TradedItem itemStack2 = new TradedItem(Items.EMERALD, j);
    ...
    public static class ProcessAndEnchantItemFactory implements Factory {
        ...
        public TradeOffer create(Entity entity, Random random) {
            ...
            DynamicRegistryManager dynamicRegistryManager = entity.getWorld().getRegistryManager();
            Optional<RegistryEntryList.Named<Enchantment>> optional = dynamicRegistryManager.getOrThrow(RegistryKeys.ENCHANTMENT).getOptional(EnchantmentTags.ON_TRADED_EQUIPMENT);
            ItemStack itemStack = EnchantmentHelper.enchant(random, this.processed, i, dynamicRegistryManager, optional);
            int j = Math.min(this.basePrice + i, 64);
            TradedItem emeraldStack = new TradedItem(Items.EMERALD, j);
            return new TradeOffer(emeraldStack, Optional.of(this.toBeProcessed), itemStack, this.maxUses, this.experience, this.multiplier);
         }
