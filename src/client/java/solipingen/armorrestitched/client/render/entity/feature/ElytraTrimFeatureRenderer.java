...
public class ElytraTrimFeatureRenderer<T extends LivingEntity, M extends EntityModel<T>> extends FeatureRenderer<T, M> {
    ...
    public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T livingEntity, float f, float g, float h, float j, float k, float l) {
        ...
        Optional<ArmorTrim> trimOptional = ArmorTrim.getTrim(livingEntity.getWorld().getRegistryManager(), itemStack, true);
        ...
        VertexConsumer vertexConsumer = sprite.getTextureSpecificVertexConsumer(ItemRenderer.getDirectItemGlintConsumer(vertexConsumerProvider, TexturedRenderLayers.getArmorTrims((trimOptional.get().getPattern().value()).decal()), true, itemStack.hasGlint()));
    ...
    private Identifier getTrimTexture(DynamicRegistryManager registryManager, ItemStack stack, boolean renderBl) {
        Optional<ArmorTrim> trimOptional = ArmorTrim.getTrim(registryManager, stack, true);
